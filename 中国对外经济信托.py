...
# 爬取内容存入文件
def input_file(download_soup_texts,each_company,f):
    print('正在保存的报告————>' + each_company['name'])
    f.write(each_company['name'])
    f.write('\n\n')
...
def file_exist():
    #创建文本文件夹
    if not os.path.exists('F:/中国对外经济信托'):
        os.mkdir('F:/中国对外经济信托')
    with open('F:/中国对外经济信托/num.txt','a',encoding = 'utf-8') as a: # 初次运行，创建页面记录文件，方便下次运行
        a.write("")
    with open('F:/中国对外经济信托/num.txt','r',encoding = 'utf-8') as a:
        out_num = a.read()
    if out_num == '':
        out_num = 0
    out_num = int(out_num) 
...
def output(before,num):
    for every_page in range(before,num):
        ...
        # 报告目录对应网址
        url = 'http://www.fotic.com.cn/l/8084-22086-'+str(every_page+1)+'.html'
        soup = gethtmltext(url)
        # 找到报告对应标签并爬取所有的报告
        soup_texts = soup.find_all('div',class_='notice-box-l')
        #print(soup_texts[0])
        # 报告网址字典
        report_urls = {}
        # 对报告目录页循环访问并打印名称及网址
        for each in soup_texts:
            # 每个报告对应网址
            download_url = each.a.get('href')
            #print(download_url)
            download_soup = gethtmltext(download_url)
            each_company = {}
            # 获取每个报告标题
            each_company['name'] = each.a.get('title')
            #print(each_company['name'])
            report_name = re.findall(r'.*',each_company['name']) # 正则匹配得到报告标题
            #print(report_name)
            # 报告列表不空，将报告网址存入网址记录文件
            if report_name!=[]:
                tilte_files = open('F:/中国对外经济信托/网址记录.txt','a+',encoding = 'utf-8')
                ...
                if report_urls['url'] not in tilte_files_urls:
                    ...
                    #打开文件，并以报告名称命名
                    report_files = open('F:/中国对外经济信托/'+ report_name[0] +'.txt','w',encoding = 'utf-8')
                    # 存入文件函数
                    input_file(download_soup_texts,each_company,report_files)
                    report_files.close()
                    time.sleep(2)
            with open('F:/中国对外经济信托/num.txt','w',encoding = 'utf-8') as f: # 每次把爬取的当前页码存到页面记录文件中
